...
export class BattleSystem {
    ...
    initializeBattleWithPlayerDeck(playerFaction, deckManager) {
        ...
        this.gameState.playerDeck = this.getPlayerActualDeck(playerFaction, deckManager);
        
        if (this.gameState.playerDeck.length === 0) {
            throw new Error(`No cards in ${GAME_CONFIG.FACTIONS[playerFaction].name} deck. Go to Manage to build your deck first!`);
        }
    ...
    generateEnemyDeck() {
        ...
        this.gameState.enemyFaction = factions[Math.floor(Math.random() * factions.length)];
        
        // Generate a complete enemy deck
        this.gameState.enemyDeck = this.generateRandomDeck(this.gameState.enemyFaction);
    ...
    // Get complete deck for a faction
    getCompleteDeck(faction) {
    ...
    calculateDeckQuality(deck) {
        ...
        deck.forEach(card => {
            totalPower += card.getPower ? card.getPower() : this.getRankValue(card.rank);
            promotionBonus += (card.stars || 0) * 2;
        });
    ...
    startBattle() {
        ...
        const overallWinner = this.gameState.handsWon.player > this.gameState.handsWon.enemy ? 'player' : 'enemy';
