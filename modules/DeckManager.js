...
export class DeckManager {
    ...
    generateCards(faction, count = 20) {
        ...
        if (this.generatedFactions.has(faction)) {
            throw new Error(`Cards have already been generated for ${GAME_CONFIG.FACTIONS[faction].name}. You can only generate cards once per faction.`);
        }
        ...
        for (let i = 0; i < count; i++) {
            ...
            const card = new Card(faction, rank, council);
            cards.push(card);
        }
    ...
    mergeCards(cardId1, cardId2) {
        ...
        if (card1.rank !== card2.rank || card1.council !== card2.council || card1.faction !== card2.faction) {
            throw new Error('Cards must be identical to merge');
        }
        
        // Add XP to first card (we'll implement XP system later)
        card1.xp = (card1.xp || 0) + 100;
        
        // Remove second card from collection
        for (const faction of FACTION_NAMES) {
            const index = this.cardCollection[faction].findIndex(card => card.id === cardId2);
            if (index !== -1) {
    ...
    addCardToDeck(faction, cardId) {
        const cardIndex = this.cardCollection[faction].findIndex(card => card.id === cardId);
        if (cardIndex === -1) {
        ...
        if (this.playerDecks[faction][card.rank][card.council] !== null) {
            throw new Error(`Slot already occupied! You can only have one ${card.rank} from ${card.council} council.`);
        }
    ...
    getCouncilDistribution(cards) {
        ...
        Object.keys(GAME_CONFIG.COUNCILS).forEach(council => {
            distribution[council] = cards.filter(card => card.council === council).length;
        });
        return distribution;
    }
    ...
    getCard(cardId) {
        for (const faction of FACTION_NAMES) {
            ...
            const card = this.cardCollection[faction].find(card => card.id === cardId);
            if (card) return card;
        }
        return null;
    }
    ...
    saveToStorage() {
        ...
        FACTION_NAMES.forEach(faction => {
            ...
            Object.keys(this.playerDecks[faction]).forEach(rank => {
                ...
                Object.keys(this.playerDecks[faction][rank]).forEach(council => {
                    const card = this.playerDecks[faction][rank][council];
                    data.playerDecks[faction][rank][council] = card ? card.toJSON() : null;
                });
            });
            
            data.cardCollection[faction] = this.cardCollection[faction].map(card => card.toJSON());
        });
                ...
                FACTION_NAMES.forEach(faction => {
                    if (data.playerDecks && data.playerDecks[faction]) {
                        ...
                        Object.keys(data.playerDecks[faction]).forEach(rank => {
                            ...
                            Object.keys(data.playerDecks[faction][rank]).forEach(council => {
                                const cardData = data.playerDecks[faction][rank][council];
                                this.playerDecks[faction][rank][council] = cardData ? Card.fromJSON(cardData) : null;
                            });
                        });
                    } else {
                    ...
                    if (data.cardCollection && data.cardCollection[faction]) {
                        this.cardCollection[faction] = data.cardCollection[faction].map(cardData => Card.fromJSON(cardData));
                    }
                });
    ...
    getAverageAnte() {
        const totalAnte = Object.values(this.deckAntes).reduce((sum, ante) => sum + ante, 0);
